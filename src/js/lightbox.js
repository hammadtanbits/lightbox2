...
(function (root, factory) {
  ...
  // Attach event handlers to the new DOM elements. click click click
    ...
    this.$lightbox.find('.lb-download').on('click', function (e) {
      ...
      var download = e.target.download;
      var canvas = document.createElement('canvas');
      var ctx = canvas.getContext('2d');
      var img = document.querySelector("#lightbox .lb-image");
      img.crossorigin= 'anonymous';
      // canvas.width = img.width;
      // canvas.height = img.height;
      // ctx.drawImage(img, 0, 0);
      var lnk = document.createElement('a');  // create an "off-screen" anchor tag
      lnk.download = download; // the key here is to set the download attribute of the a tag
      lnk.href = link;
      lnk.target = '_blank';
      console.log(link);
